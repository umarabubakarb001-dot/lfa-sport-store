...
export default function Login({ onLoginSuccess, theme, onThemeToggle }: LoginProps) {
  ...
  const [password, setPassword] = useState('lfa2026'); // Prefilled default
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              disabled={loading}
              id="btn-login"
              className="w-full py-3 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 disabled:from-slate-800 disabled:to-slate-800 disabled:text-slate-500 text-slate-950 font-black rounded-xl text-xs tracking-wider transition-all shadow-lg cursor-pointer uppercase flex items-center justify-center gap-2"
            >
              {loading ? 'Authenticating Access...' : 'Log In'}
            </button>
          </form>

          <p className="text-[10px] text-slate-500 text-center tracking-wide leading-relaxed">
